...
if (path_1.default.join(__dirname).split("\\").pop() == "utils") {
    backgroundImagePath = path_1.default.join(__dirname, 'bj.png');
}
else {
    backgroundImagePath = path_1.default.join(__dirname, 'bj.png');
}
...
async function generateThumbnails(ctx, options = {}) {
    const { inputDir: inDir = inputDir, colorDir: colDir = colorDir, grayDir: grayOutDir = grayDir, width = 80, height = 80 } = options;
    // 确保目录存在
    ...
    // 清空输出目录
    fs_1.default.readdirSync(colDir).forEach(file => fs_1.default.unlinkSync(path_1.default.join(colDir, file)));
    fs_1.default.readdirSync(grayOutDir).forEach(file => fs_1.default.unlinkSync(path_1.default.join(grayOutDir, file)));
    const files = fs_1.default.readdirSync(inDir).filter(name => /\.(png|jpe?g)$/i.test(name));
    const tasks = files.flatMap(async (file) => {
        const inputPath = path_1.default.join(inDir, file);
        const baseName = path_1.default.parse(file).name + '.png';
        const colorOut = path_1.default.join(colDir, baseName);
        const grayOut = path_1.default.join(grayOutDir, baseName);
        // 先生成彩色缩略图，宽度80，高度自适应，保持原比例
        const colorBuffer = await (0, sharp_1.default)(inputPath)
            ...
            .png()
            .toBuffer();
        await (0, sharp_1.default)(colorBuffer).toFile(colorOut);
        // 再生成灰度缩略图
        const grayBuffer = await (0, sharp_1.default)(inputPath)
            .resize({ width: width, fit: 'inside', background: { r: 0, g: 0, b: 0, alpha: 0 } })
            .grayscale()
            .extend({
            top: 2, bottom: 2, left: 2, right: 2,
            ...
            .png()
            .toBuffer();
        await (0, sharp_1.default)(grayBuffer).toFile(grayOut);
    });
    await Promise.all(tasks);
    ctx.logger.info('缩略图初始化完成');
    ctx.logger.info('✅ 缩略图已生成（彩色 + 灰度）');
    return {
        colorFiles: files.map(file => path_1.default.parse(file).name + '.png'),
        colorDir: colDir,
        grayDir: grayOutDir
    };
}
...
async function generateMixedBackgroundImage(ctx, config, colorImageNames, options = {}) {
    const { backgroundPath = backgroundImagePath, colorDir: colDir = colorDir, grayDir: grayOutDir = grayDir, imageSize = 80, gridWidth = 567, padding = 3 } = options;
    // 获取所有图片文件
    const grayFiles = fs_1.default.readdirSync(grayOutDir).filter(f => /\.(png|jpe?g)$/i.test(f));
    const thumbFiles = [...grayFiles]; // 复制文件列表用于后续处理
    // 读取所有缩略图的尺寸，找出最大高度
    const imageMetas = await Promise.all(thumbFiles.map(async (file) => {
        const baseName = path_1.default.parse(file).name.split(config.wifeNameSeparator)[1];
        const isColor = colorImageNames.includes(baseName);
        const imgPath = isColor ? path_1.default.join(colDir, file) : path_1.default.join(grayOutDir, file);
    ...
    const bgResized = await (0, sharp_1.default)(backgroundPath)
        .resize({ width: gridWidth, height: totalHeight })
        .toBuffer();
    // 直接返回图片的二进制数据
    const imageBuffer = await (0, sharp_1.default)(bgResized)
