...
const affection_1 = require("./affection");
const MIGRATION_KEY = "schemaVersion";
exports.CURRENT_SCHEMA_VERSION = "1.5.0";
...
async function getStoredSchemaVersion(ctx) {
    const rows = await ctx.database.get("wifeMigration", { key: MIGRATION_KEY });
    return rows[0]?.value ?? "0.0.0";
}
async function setStoredSchemaVersion(ctx, version) {
    const rows = await ctx.database.get("wifeMigration", { key: MIGRATION_KEY });
...
async function migrateTo150(ctx) {
...
async function runDatabaseMigrations(ctx) {
