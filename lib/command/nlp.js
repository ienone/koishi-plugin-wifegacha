...
function nlp(ctx, config) {
    ctx
        ...
        if (!userId) {
            // 提示用户@要牛的群友
            send([(0, koishi_1.h)("quote", { id: session.messageId }), "请@要牛的群友"]);
            return;
        }
        ...
        // 获取老婆数据
        const wifeData = (await ctx.database.get("wifeData", { name: targetUserData.wifeName }))[0];
        ...
        if (!targetUserData || !targetUserData?.wifeName) {
            send([(0, koishi_1.h)("quote", { id: session.messageId }), "对方还没有老婆"]);
            return;
        }
        // 获取对方老婆好感等级
        ...
        // 更新老婆数据
        await utils_1.default.createGroupWifeData(ctx, session, targetUserData.wifeName);
        ...
        const targetWifeAffection = targetUserData.wifeHistories.find((item) => item.wifeName === targetUserData.wifeName)?.affection ?? 0;
        // const targetTodayAffection = targetUserData.todayAffection.find(
        //   (item) => item.wifeName === targetUserData.wifeName
        // )?.todayAffection ?? 0;
        const affection = myUserData.wifeHistories.find((item) => item.wifeName === targetUserData.wifeName)?.affection ?? 0; // 默认为 0
        ...
        // 生成成功率
        const successRate = utils_1.default.camelCase(lpNum, ntrSuccessCount, targetWifeNum, targetaffectionLevel, 
        // targetTodayAffection,
        affection, targetWifeAffection);
        // 概率计算方式
