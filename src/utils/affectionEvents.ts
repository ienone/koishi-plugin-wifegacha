...
function normalizeCustomRule(rule: NonNullable<Config["customAffectionEvents"]>[number]): AffectionEventRule | undefined {
  if (!rule?.id || !rule?.message) return undefined;
  const actions = (rule.actions ?? [])
    .filter((action): action is AffectionInteractionAction => actionSet.has(action));
...
function applyDelta(baseDelta: number, rule: AffectionEventRule, action: AffectionInteractionAction) {
  let next = baseDelta;
  if (rule.deltaMode === "add") next = baseDelta + rule.deltaValue;
  if (rule.deltaMode === "multiply") next = Math.round(baseDelta * rule.deltaValue);
  if (rule.deltaMode === "set") next = rule.deltaValue;
  if (rule.effects?.failAction) next = 0;
  return clamp(next, action === "date" ? -10 : -6, action === "date" ? 15 : 8);
}

export function rollAffectionEvent(config: Config, action: AffectionInteractionAction, baseDelta: number): RolledAffectionEvent {
  const noEvent = {
    id: "default",
    delta: baseDelta,
  ...
  if (!config.affectionEventSwitchgear) return noEvent;
