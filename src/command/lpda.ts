...
function groupRecord(wife, groupId: string) {
...
async function handleGroupWifeArchive(ctx: Context, config: Config, session, wifeName?: string) {
  ...
  if (diffSetonds < config.lpdaDateInterval) {
    ...
    await send([h("quote", { id: session.messageId }), `档案查询冷却中，${Math.floor(remain / 60)}分${remain % 60}秒后可以再次查询`]);
    return;
  }
  ...
  if (wifeName) {
    ...
    if (!wife) {
      ...
      await send([h("quote", { id: session.messageId }), possible.length ? `没有找到精确老婆名。你可能想找：\n${possible.join("\n")}` : "老婆不存在"]);
      return;
    }
    await utils.createGroupWifeData(ctx, session, wife.name);
    ...
    await send([h("quote", { id: session.messageId }), lines.join("\n"), wife.filepath ? h.image(pathToFileURL(wife.filepath).href) : ""]);
    return;
  }

  const wivesInGroup = allWives.filter((wife) => wife.groupData?.some((item) => item.groupId === groupId));
  if (!wivesInGroup.length) {
    await send([h("quote", { id: session.messageId }), "本群还没有老婆档案"]);
    return;
  }
  const topBy = (field: "drawCount" | "ntrCount" | "ntrFailCount" | "divorceCount" | "fuckCount") => {
